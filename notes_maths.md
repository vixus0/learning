...
*Definition* A graph G is a pair of sets (V,E) where V is a non-empty
...
*Definition* Two nodes Xi and Xj are _adjacent_ if {Xi,Xj} is in E.

*Definition* An edge e = {Xi,Xj} is _incident_ to Xi and Xj.

*Definition* The number of edges incident to a node is called its _degree_.

*Definition* A graph is _simple_ if it has no loops or multiple edges. A loop
...
*Definition* Minimum # required colours K is the graph's _chromatic number_, $(s).
...
*Definition* A graph G(V,E) is _bipartite_ if V can be split into Vl, Vr so that
